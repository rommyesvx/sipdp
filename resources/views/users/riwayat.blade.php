...
<div class="container py-5">
    ...
    <div class="history-table">
        <div class="row header-row text-muted small fw-bold mb-3 d-none d-lg-flex">
            ...
            <div class="col-lg-3">
                @php $tujuanDirection = ($sort == 'tujuan' && $direction == 'asc') ? 'desc' : 'asc'; @endphp
                <a href="{{ url()->current() }}?sort=tujuan&direction={{ $tujuanDirection }}">
                    Tujuan Permohonan
                    @if($sort == 'tujuan')<i class="fas fa-sort-{{ $direction == 'asc' ? 'up' : 'down' }} ms-1"></i>@endif
                </a>
            </div>
            <div class="col-lg-2">Kriteria Data</div>
            <div class="col-lg-1">
                ...
                <a href="{{ url()->current() }}?sort=created_at&direction={{ $dateDirection }}">
                    Tgl Diajukan
                    @if($sort == 'created_at')<i class="fas fa-sort-{{ $direction == 'asc' ? 'up' : 'down' }} ms-1"></i>@endif
                </a>
            </div>
            <div class="col-lg-2">
                ...
                <a href="{{ url()->current() }}?sort=status&direction={{ $statusDirection }}">
                    Status
                    @if($sort == 'status')<i class="fas fa-sort-{{ $direction == 'asc' ? 'up' : 'down' }} ms-1"></i>@endif
                </a>
            </div>
        ...
        <div class="row history-row align-items-start mb-3 position-relative">
            ...
            <div class="col-lg-2 mb-2 mb-lg-0">
                <strong class="d-lg-none">ID: </strong>
                <span class="fw-semibold">{{ $data->nomor_permohonan }}</span>
            </div>
            ...
            <div class="col-lg-2 mb-3 mb-lg-0">
                ...
                @elseif(in_array($data->status, ['ditolak', 'ditolak kepala bidang']))
            ...
            <div class="col-lg-1 text-center">
                <div class="action-icons d-flex justify-content-start justify-content-lg-center gap-3">
                    <a href="{{ route('permohonan.show', $data->id) }}" title="Lihat Detail" onclick="event.stopPropagation();" class="position-relative z-1">
                        <i class="fas fa-eye"></i>
                    </a>
                    @if ($data->status == 'selesai' && $data->fileHasil)
                    <a href="{{ route('permohonan.downloadHasil', ['id' => $data->id]) }}" title="Unduh Hasil" onclick="event.stopPropagation();" class="position-relative z-1">
